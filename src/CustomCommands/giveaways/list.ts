...
export default (client: ClientType) =>
	new CustomCommands({
		props: {
			name: 'çekiliş-liste',
			aliases: ['giveaway-lists', 'çekiliş-listesi', 'çekilişliste', 'çekilişlistesi'],
			category: 'Çekiliş',
		...
		execute: (message: Message, args: string[]) => {
			...
			giveaways2.forEach((thisGiveaway: Giveaway, i: number) => {
				giveaways.push(
					`\`${i + 1}.\` <#${thisGiveaway.channelId}> | **${thisGiveaway.winnerCount}** Kazanan | Ödül **${
						thisGiveaway.prize
					}** | [Gitmek için tıkla!](https://discord.com/channels/${message.guild?.id}/${
						thisGiveaway.channelId
