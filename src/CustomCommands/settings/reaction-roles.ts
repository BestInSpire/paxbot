	...
	new CustomCommands({
		...
		execute: async (message: Message, args: string[]) => {
			const buttonsRow = new MessageActionRow().addComponents(
				...
				new MessageButton().setCustomId('delete').setLabel('Sil').setStyle('DANGER'),
				new MessageButton().setCustomId('list').setLabel('Listele').setStyle('PRIMARY'),
			);
					...
					buttonsCollector.on('collect', async (i) => {
						if (i.customId == 'add') {
							msg.edit({
								content: null,
								components: [],
								embeds: [embedCreator("Tepki Rol'ün bulunacağı kanalı etiketle!")],
							}).then(() => {
								const awaitMessageFilter = (m: Message<boolean>) => m.author.id == message.author.id;
								msg.channel
									.awaitMessages({ filter: awaitMessageFilter, max: 1, time: 2 * 60 * 1000 })
									.then(async (channelAnswer) => {
										await channelAnswer.first()?.delete();
										const channel = channelAnswer.first()?.mentions.channels.first();

										msg.edit({
											content: null,
											components: [],
											embeds: [embedCreator("Tepki Rol'ün ayarlanacağı rolü etiketle!")],
										}).then(() => {
											const awaitMessageFilter = (m: Message<boolean>) =>
											...
											msg.channel
												...
												.then(async (roleAnswer) => {
													...
													const role = roleAnswer.first()?.mentions.roles.first();

													msg.edit({
														content: null,
														components: [],
														embeds: [
															embedCreator("Tepki Rol'ün ayarlanacağı emojiyi yaz!"),
														],
													}).then(() => {
														const awaitMessageFilter = (m: Message<boolean>) =>
															m.author.id == message.author.id;
														msg.channel
															...
															.then(async (emojiAnswer) => {
																...
																const emoji = emojiAnswer.first()?.content;
																...
																msg.edit({
																	...
																	msg.channel
																		...
																		.then(async (messageidAnswer) => {
																			await messageidAnswer.first()?.delete();
																			const messageId =
																				messageidAnswer.first()?.content;
																				...
																				.then(
																					async (
																						...
																						if (isCustomEmoji(`${emoji}`)) {
																							...
																							const searchEmoji:
																								| GuildEmoji
																								| undefined = message.guild?.emojis.cache.find(
																								(x) =>
																									x.name == emojiName,
																							);
																							const isData =
								...
								.then((x) => {
									...
									const content = data.map(
										(
											...
											} => <@&${x.role}>  ([Mesaja Git!](https://canary.discord.com/channels/${
												message.guild?.id
											}/${x.channel}/${x.message}))\n`;
										},
									);
									...
									data.length < 1
										...
										: msg
												...
												.then(async (msgxd) => {
													const filter = (i: Message<boolean>) =>
														i.author.id == message.author.id;
													msgxd.channel
														.awaitMessages({ filter, max: 1, time: 2 * 60 * 1000 })
														.then(async (answer) => {
															const selected =
																data[parseInt(`${answer.first()?.content}`) - 1];
