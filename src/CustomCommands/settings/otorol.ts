...
export default (client: ClientType) =>
  new CustomCommands({
    props: {
      ...
      category: "Ayarlar",
      description:
        "Sunucuya yeni katılan üyelere otomatik verilecek rolü ayarlarsınız!",
      usage: "!otorol @rol",
      enabled: true,
      owner: false,
      nsfw: false,
    ...
    execute: async (message: Message, args: string[]) => {
      if (!args[0])
        return message.reply({
          content:
            "Yeni katılan üyelere otomatik olarak verilecek rolü etiketlemelisin!",
        });
      if (!args[0].includes("@"))
        return message.reply({
