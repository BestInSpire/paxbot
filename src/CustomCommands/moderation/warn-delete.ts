          ...
          .then((msg: Message) => {
            ...
            collector.on("collect", async (i) => {
              const selectedData = data.warns.find(
                (x: any) => x.globalTime == parseInt(i.values[0])
              );

              let datawarns = data.warns;
              let indexof = datawarns.indexOf(selectedData);
              datawarns = datawarns.filter(
                (x: any) => x.globalTime !== parseInt(i.values[0])
              );
              datawarns.map((_: any, g: number) => {
                if (g < indexof) {
                  datawarns[g].time = datawarns[g].time - selectedData.time;
                  datawarns[g].globalTime =
                    datawarns[g].globalTime - selectedData.time;
                  if (datawarns[g + 1])
                    datawarns[g].globalTime =
                      datawarns[g].globalTime + datawarns[g + 1].time;
                }
              });
              data.warns = null;
              data.save().then(() => {
                data.warns = datawarns;
                data.save().then(async (_x: any) => {
                  if (
                    client.guilds.cache
