...
export default (client: ClientType) =>
	new CustomCommands({
		props: {
			name: 'uyarılar',
			aliases: ['uyarı-listesi', 'warn-list', 'uyarı-liste', 'uyarılistesi', 'uyarıliste', 'warnlist'],
			category: 'Moderasyon',
			description: 'Etiketlediğiniz kullanıcının veya kendinizin uyarı listesini görüntülersiniz!',
			usage: '!uyarılar [@üye]',
			enabled: true,
			owner: false,
			nsfw: false,
		...
		execute: async (message: Message<boolean>, args: string[]) => {
			...
			if (data && data.warns.length > 0) {
				const list = data.warns
					...
					.join('\n\n');
				const embed = new MessageEmbed().setColor('#fbbd08').setDescription(`${list}`);
				message.reply({
					embeds: [embed],
				});
			} else {
