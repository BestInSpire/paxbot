...
export default (client: ClientType) =>
	new CustomCommands({
		...
		execute: async (message: Message, args: string[]) => {
			...
			if (!args[1])
				return message.reply({
					content: 'Bir süre girmelisin! Örn: `!sustur @kullanıcı 15m [Sebep]` (15 dakika)',
				});
			if (isNaN(ms(args[1])))
				return message.reply({
					content: 'Geçerli bir süre girmelisin! Örn: `!sustur @kullanıcı 15m [Sebep]` (15 dakika)',
				});
			...
			await new dataModels.mute({
				...
				.then(() => {
					...
					client.users.cache.get(`${message.mentions.users.first()?.id}`)?.send({
						embeds: [
							new MessageEmbed()
