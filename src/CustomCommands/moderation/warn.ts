...
export default (client: ClientType) =>
  new CustomCommands({
    props: {
      ...
      category: "Moderasyon",
      description:
        "Etiketlediğiniz kullanıcıyı belirttiğiniz süre ve sebeple uyarırsınız!",
    ...
    execute: async (message: Message, args: string[]) => {
      ...
      if (!args[1])
        return message.reply({
          content:
            "Bir süre girmelisin! Örn: `!uyarı @kullanıcı 15m [Sebep]` (15 dakika)",
        });
      if (isNaN(ms(args[1])))
        return message.reply({
          content:
            "Geçerli bir süre girmelisin! Örn: `!uyarı @kullanıcı 15m [Sebep]` (15 dakika)",
        });
      ...
      if (!data) {
          ...
          warns: [
            {
              ...
              msFormatted: humanizeDuration(msFormatted, {
                round: true,
                language: "tr",
              }),
          ...
          .then(() => {
            ...
            client.users.cache.get(`${`${mention?.id}`}`)?.send({
              embeds: [
                new MessageEmbed()
                  .setAuthor({
                    name: message.guild?.name + " Sunucusunda Uyarıldın",
                  })
                  .addField(
                    "Sebep:",
        ...
        datawarns.map((_: any, i: number) => {
          if (
            datawarns[i + 1] &&
            datawarns[i].globalTime == datawarns[i + 1].globalTime
          )
            datawarns[i].globalTime =
              datawarns[i].globalTime + datawarns[i + 1].time - msFormatted;
        });
