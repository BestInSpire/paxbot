...
export default (client: ClientType) =>
	new EventStructure({
		...
		execute: async (member: GuildMember, oldChannel: VoiceChannel, newChannel: VoiceChannel) => {
			const data = await dataModels.modlog.findOne({
				guild: member.guild?.id,
			});
			if (!data || !data.isActive || !data.settings.voiceChannelSwitch) return;
			...
			const embed = new MessageEmbed()
				...
				.addField('Sesli kanalını değiştiren kullanıcı:', `${member.user}`, true)
