...
import { BitField, Message, PermissionString, RecursiveReadonlyArray, MessageEmbed } from 'discord.js';
	...
	new EventStructure({
		...
		execute: async (message: Message) => {
			...
			if (message.content.startsWith(prefix)) {
				let commandName: string = message.content.split(' ')[0].slice(prefix.length);
				let command =
					//@ts-ignore
				...
				if (command) {
					if (!command.props.enabled && !developers.includes(message.author.id))
						return message.reply('Bu komut geçici olarak devredışı bırakılmış!');
					if (command.props.owner && !developers.includes(message.author.id))
						return message.reply('Bu komutu kullanmak için bot geliştiricisi olmalısın!');
					//@ts-ignore
					...
					try {
						...
						/**@ts-ignore */
						commandCooldown[message.author.id][command.props.name] = Date.now() + command.props.cooldown;
					} catch (err) {
						console.error(err);
					}
				}
