...
export default (client: ClientType) =>
	new EventStructure({
		...
		execute: (interaction: Interaction) => {
			...
			if (command) {
				if (command.data.userType == 'developers' && !developers.includes(interaction.user.id))
					return interaction.reply({
						content: 'Bu komutu sadece geliştiriciler kullanabilir!',
					});

				if (command.data.userType == 'premium' && !developers.includes(interaction.user.id))
					return interaction.reply({
						content: 'Bu komut premium kullanıcılar için!',
					});

				if (command.data.userType == 'voting' && !developers.includes(interaction.user.id))
					return interaction.reply({
